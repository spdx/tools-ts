...
export const getPackagePath = async (
  ...
  const entry =
    yarnState[stringifiedLocator] || yarnStateAliases[stringifiedLocator];
...
export const _getYarnStateAliases = (yarnState: YarnState): YarnState => {
  return Object.entries(yarnState).reduce<YarnState>(
    (acc, [stringifiedLocator, yarnStateValue]) => {
